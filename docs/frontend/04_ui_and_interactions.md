...
* **The Global Dock:** A floating, pill-shaped dock centered at the bottom of the screen. The dock contains a **3-way Lens Toggle** (Canvas / Link / Matrix) followed by a separator, then contextual tools that change based on the active lens:
  * *Canvas Lens:* Display ▾ (unified visibility toggles for Nard Types + Connection Types), Sticky (drag source), Add ▾ (creation grid for nords/connections + **Manage Types** button).
  * *Link Lens:* Relationship ▾ (active connection type selector with spectrum slider), Context toggle (show/hide unconnected nords at 20%), Connect (crosshair draw mode), Sticky.
...
* **Hover-Focus (Graph Isolation):** Hovering over a Nard dims all non-connected Nords to 20% opacity, highlighting the active local graph path brightly.
...
* **Rigid Group Drag:** Move the entire selection as a locked formation, preserving internal distances between selected Nords while the physics engine recalculates their external Tethers.
...
* **Bulk Delete:** Remove all selected Nords and their associated Tethers.
...
* **Multi-Line Display:** If the active Line Type connects the dragged Nard to multiple other Nords, the panel shows a compact stack of all affected connections with their live values.
...
* **Per-Nard Threads:** Each Nard has its own threaded conversation. Users and AI agents can post comments, @mention teammates, and attach inline references to other Nords or Snapshots.
