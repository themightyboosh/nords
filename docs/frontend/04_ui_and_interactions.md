...
* **Mobile Responsive (≤768px):** Header collapses to project name + hamburger menu. Dock becomes a two-tier horizontal bar floating at the bottom (safe-area-inset protected). The top tier is a full-width segmented control (Canvas / Link / Matrix). The bottom tier is an evenly spaced horizontal row of icon-only contextual tools. Labels and separators are hidden to optimize space.
...
* **Line Paths & Spreading (Ribboning):** Euclidean distance strictly governs routing. **Orthogonal/A* avoidance pathfinding is fully rejected** because it artificially inflates line length, breaking the visual distance invariant. Parallel connections between the same two Nords use pure Quadratic Bézier curves with a perpendicular offset to "ribbon" outward. Single connections use straight direct lines.
