...
* **Entities (Nords):** Data objects with customizable metadata schemas (defined via Nord-Builder). These act as the nodes in the spatial graph.
* **Connections (Lines):** Relationships between Entities. These act as the edges in the graph, utilizing the 0.0 to 1.0 spatial paradigm.
...
A nord is the atomic unit. It is a rich content object — closer to a Notion page than a sticky note.
Nord types are user-defined schemas (e.g., "Task", "Person", "Risk", "Idea"). Each type defines which fields are present. Types are managed by Admins.

#### Nord Metadata Options (Node Schema)
The "Nord-Builder" allows users and AI agents to configure custom schemas for different Nord types.
...
**Optional Data Primitives (Type-Level Properties):** 
Familiar to any Notion power user, property schemas are defined at the **type level** via Manage Types. Instance Nords inherit the properties defined by their type — individual instances cannot add or remove properties. Available property types:
...
#### Line Metadata Options (Connection Schema)
Because a Line's primary data payload is its spatial distance, additional metadata remains lightweight to prevent canvas clutter. As with Nords, property schemas are defined at the **type level** via Manage Types — individual connection instances cannot add or remove properties.
...
### 1.4 The Semantic Stage (Qualitative Translation)
...
* **Custom Breakpoints:** Because qualitative concepts are rarely evenly distributed (e.g., "Critical" might only span 0.0-0.1, while "Normal" spans 0.1-0.9), the Stage supports user-adjustable breakpoints. In the Line Library, the label dividers render as draggable slider handles.
* **Bi-Directional UI Sync:**
  * **Visual Dragging (Graph to Data):** If a user drags two Nords further apart, the system calculates the new math value (e.g., 0.85) and automatically updates the Connection's visible UI label.
  * **Menu Selection (Data to Graph):** If a user clicks the Connection's metadata menu and changes the stage from "Loves" to "Hates," the physics engine immediately forces the Nords physically apart on the canvas to match the new mathematical reality.
...
* **Auto-Equilibrium:** If a user forces a Nord into a position that mathematically conflicts with its other active lines, the system calculates the equilibrium. Connected Nords will "pull" or "relax" to balance the tension.
* **Z-Index Collision Avoidance:** In dense clusters, localized repulsion fields prevent Nords from achieving 100% overlap. Z-index is dynamically sorted by the Nord's structural "Size" property.
...
* **Zero-Gravity:** If a Nord has no connections, or its only active lines are toggled to invisible/inactive, it is unmoored. It will simply float in its absolute resting X/Y coordinates.
...
The canonical export/import engine runs purely on structured JSON payloads mapping standard graph-edges semantics (Nord Types, Line Types, the graph arrays, Views/Filters arrays, and Snapshots). Under the hood, this relies on Postgres utilizing graph/relational patterns.

### 2.2 Immutable Snapshots (State Preservation)
A Snapshot is an explicitly wrapped keyframe of the entire canvas.
* **Scope:** It captures exact coordinates, line values (0.0-1.0), and the exact metadata payloads of every Nord and Line at a precise millisecond.
...
To prevent generating corrupted snapshots when users modify schemas globally (e.g., deleting a "Status" property on a Nord Template), the system leverages a "Soft Delete" data pipeline.
...
### 2.4 The Temporal Player & Snapshot Viewer
Snapshots function natively as a presentation and storytelling medium. The **Project Settings → Snapshots** section provides the primary management interface.
* **Management:** Each snapshot is explicitly named with an optional markdown description. Load, Export (JSON), and Delete actions per snapshot. **Snapshots can also be taken directly from the Global Dock** via the Snapshot button (available in all lens modes).
* **The Playback Mechanic ("Animate Through"):** Clicking 'Animate Through All' in the Snapshot Viewer triggers an automated sequence through the chronological history.
...
### 2.5 Full Export (RAG Context)
The entire project can be exported as a single structured document optimized for RAG ingestion and LLM context windows. Includes: project metadata, all nords with full properties, all connections with distance values and stage labels, graph topology (Mermaid representation), spatial coordinates, all comments, and snapshot metadata. Available in Markdown (LLM-optimized), JSON, and YAML formats. Accessible from **Project Settings → Full Export**.

### 2.6 Snapshot Diffing
