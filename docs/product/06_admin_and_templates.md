...
* **Step 2: Project Icon Selection:** Choose a project icon from the Lucide icon library. This icon appears in the project switcher and header.
...
* **Step 4: API Token Generation (Optional):** An access token is auto-generated for MCP integrations. Users can configure token scope (Read-Only / Read-Write / Admin) in Project Settings later.
* *(Snapshots are always named, event-driven saves — no typology selection needed.)*
...
* **Sample Data Injection:** Users can toggle "Load with Sample Data". The engine populates the canvas with a pre-built, fully connectioned mock-graph so the user can immediately interact with the physics engine and see Lens transitions.
...
* **Connections:** Alignment / Contribution.
* **Mechanic:** Initiatives must physically connection to Key Results. Disconnected (orphan) work floats to the edges of the canvas, immediately highlighting rogue projects.
...
* **Connections:** Emotional Tension, Physical Proximity, Alliance.
...
* **Connections:** Blockers (Dependencies), Assignments.
* **Mechanic:** Toggling Connection visibility reorganizes the canvas from an "Assigned To" cluster into a linear "Blocker" sequence dynamically via physics.
...
* **Connections:** Semantic Similarity (Cosine Distance).
* **Mechanic:** When a Query Nord is placed, relevant Document Chunks physically snap tighter. Users visually audit / manually tweak the RAG context window by dragging documents in or out of the Query's gravity well.

## 5. Onboarding Flow (Progressive Complexity)
* **First Nord:** Create the first item natively.
...
* **The First Drag:** User drags Nord B towards A, watching the raw mathematical data entry occur via visual spatialization.
