...
export const nordTypesRepo = {
  ...
  async update(id: string, updates: Partial<Pick<NordType, 'name' | 'icon' | 'accent_color' | 'description' | 'properties_schema' | 'scale_property' | 'sort_order'>>): Promise<NordType | null> {
...
export const connectionTypesRepo = {
  ...
  async update(id: string, updates: Partial<Pick<ConnectionType, 'name' | 'verb' | 'accent_color' | 'description' | 'stroke_style' | 'direction_filter' | 'direction_prepositions' | 'measurement_mode' | 'x_stage_labels' | 'y_stage_labels' | 'properties_schema' | 'sort_order'>>): Promise<ConnectionType | null> {
    const setClauses: string[] = [];
    const values: unknown[] = [];
    let idx = 1;
